...
import { IFiterType } from '@/cmodels';

const AssessmentFilter = () => {
  const dispatch = useDispatch();
  const products: any[] = [];
  const filters: any = {};
  const checkactive = 'asc';
  ...
  const filterCategory: IFiterType[] = [
    {
      id: 1,
        ...
        {filterCategory.map((filter) => {
          ...
          return (
            <ListItemButton
              ...
              selected={filters.category === `${filter.sort}`}
              ...
              if (curColor !== 'All') {
                return (
                  <Avatar
                    ...
                    // }
                  >
                    {filters.color === curColor ? <IconCheck size="13" /> : ''}
                  </Avatar>
                );
              } else {
